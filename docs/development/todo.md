...
- 🔄 **C3 圖表**：
  ...
  - ✅ **C3-2b 實盤/影子每日權益曲線**（2026-07-15）：新增 `equity_snapshots` 表（純 `cash_ledger`+`fills` 重播、不依賴 `PortfolioProjection` 的「現在」狀態）；`DailySimulationRunner.run_daily()` Stage 2.5 之後每日寫入一筆。`scripts/backfill_equity_snapshots.py` 一次性回填 `simulation-main` 23 個歷史交易日，對 `data/app.db` 2026-07-14 那筆核對逐分錢吻合（cash=335,930／positions_value=182,848／total_equity=518,778）。首頁資金總覽卡下方新增折線圖，複用既有 `backtest-charts.js`（零改 JS）。範圍僅 simulation-main（國泰後續會被動累積快照但本次不回填/不顯示）。測試：`test_equity_snapshots_service.py` 8 項（含對 `cash_balances`/`build_capital_overview` 的交叉驗證不變式）+ `test_simulation_runner.py`/`test_web_server.py` 各補 1 項，unit 全綠 377。
