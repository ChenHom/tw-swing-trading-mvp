...
當前 MVP 為了流程閉環做了部分簡化，在生產部署與後續功能開發前需注意以下邊界限制：

1. **資金分配與超額分配 (PortfolioOrderAllocator)**:
   - 當前系統的委託計畫預覽與執行，是針對個別訊號獨立進行可用資金計算。在多個 BUY 訊號同時發生時，可能導致可用現金被重複/超額分配。後續需實作帳戶層級的全局資金分配與排序器。
2. **長期持有部位 (Long-Term Lot) FIFO 隔離**:
   - 目前長期持有部位與策略部位混在同一個 `position_lots` 中。在執行 SELL 出場時，資料庫的 FIFO 撮合仍會優先匹配最早買入的批次，造成長期持有部位被誤匹配而賣出。後續需於 `position_lots` 表引入 `management_mode` 或 `strategy_id` 進行物理隔離。
3. **模擬重置 (Simulation Reset) 的事實數據保護**:
   - 目前 reset 命令會清空當日狀態。在真實/模擬運行中，若已經產生成交紀錄，清空事實表將破壞不可變流水帳的嚴謹性。重置範圍應僅限於未成交/待執行狀態。
4. **交易宇宙 (Universe) 責任拆分**:
   - 手動錄入會自動修改 `universe.yaml`。這將策略選股範圍與持倉標的現價同步範圍混淆，破壞了回測重現性。未來應拆分為策略候選池與持倉估值池。
5. **排程運行的隱式解析**:
   - 在自動化排程中，為防解析混亂，必須明確指定帳戶名稱，而不應依賴動態帳戶解析。
