...
MVP 核心功能與首批架構缺陷修正已完成。後續優先開發方向包含：

1. **手動成交事實完整性與修正模型**:
   - 擴展 `record-fill` 以支援手動輸入外部券商實際手續費與交易稅，記錄來源 metadata 識別碼，並實作沖銷修正流水紀錄（reversal / corrected fill）以維持不可變 facts 的完整。
2. **排程運行的隱式帳戶解析防範**:
   - 重構排程核心命令（如 `simulation run-daily`），對於非互動環境強制指定明確的 `account_id` 與 `strategy_id`，禁止使用省略參數的隱式動態解析。
3. **零股與整張股票成交模型分流**:
...
1. **排程運行的隱式帳戶解析風險**:
   - CLI 支援省略 `--account` 進行動態解析，這方便了手動查詢，但對於自動排程 (cron) 存在選錯帳戶或任務崩潰風險。自動化排程必須強制指定明確的 `account_id` 與 `strategy_id`。
2. **手動成交的事實完整性**:
   - 目前 `record-fill` 僅能使用估計費率，且未記錄來源資訊（如 `source = MANUAL_IMPORT`、`broker`、真實手續費與稅額等），亦缺乏沖銷修正的模型支援。
3. **撮合模型與公司行動限制**:
